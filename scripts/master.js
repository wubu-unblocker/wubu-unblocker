(async () => {
    ...
    // ==========================================
    // BLOOKET CHEATS - REACT/COLYSEUS HELPERS
    // ==========================================
    // Blooket uses two main architectures:
    // 1. Legacy: Firebase + React class components with stateNode
    // 2. New: Colyseus + React functional components with hooks

    // Helper to find React Fiber key on an element
    ...
    // Find Colyseus room from React Fiber
    // The room is in memoizedState hooks, typically 1-2 levels up from .wrapper
    function getColyseusRoom() {
        // Find starting element
        const wrapper = document.querySelector('.wrapper') ||
            document.querySelector('#gameWrapper') ||
            document.querySelector('[class*="wrapper"]');
        if (!wrapper) return null;

        // Get fiber - try both methods
        const fiberKey = Object.keys(wrapper).find(k => k.startsWith('__reactFiber'));
        let fiber = fiberKey ? wrapper[fiberKey] : Object.values(wrapper)[0];

        if (!fiber || typeof fiber !== 'object' || !fiber.tag) return null;

        // Walk up the fiber tree (typically room is 1-5 levels up)
        ...
        while (current && depth < 20 && !visited.has(current)) {
            ...
            if (current.memoizedState) {
                let hookNode = current.memoizedState;
                let hookIdx = 0;

                while (hookNode && hookIdx < 30) {
                    const state = hookNode.memoizedState;

                    // Look for Colyseus Room signature
                    if (state && typeof state === 'object' && state !== null) {
                        // Room has these properties: hasJoined, onStateChange, onLeave, state
                        if (state.hasJoined !== undefined &&
                            typeof state.onStateChange === 'function' &&
                            state.state) {
                            return state; // This is the Room!
                        }

                        // Also check for room property
                        if (state.room && state.room.hasJoined !== undefined) {
                            return state.room;
                        }

                        // Check .current (useRef)
                        if (state.current && state.current.hasJoined !== undefined) {
            ...
            // Check memoizedProps for context values
            const props = current.memoizedProps;
            if (props && typeof props === 'object') {
                if (props.value && props.value.room && props.value.room.state) {
                    return props.value.room;
                }
            }
    ...
    // Get current player from Colyseus room
    ...
    // Get question data from React fiber tree
    // Question is passed as props to components, not stored in Colyseus
    function getQuestion() {
        ...
        const fiberKey = Object.keys(wrapper).find(k => k.startsWith('__reactFiber'));
        let fiber = fiberKey ? wrapper[fiberKey] : Object.values(wrapper)[0];
        if (!fiber || !fiber.tag) return null;

        // Search fiber tree for question in props
        const visited = new WeakSet();
        function searchForQuestion(f, depth = 0) {
            if (!f || depth > 20 || visited.has(f)) return null;
            visited.add(f);

            // Check memoizedProps for question
            if (f.memoizedProps && f.memoizedProps.question) {
                const q = f.memoizedProps.question;
                if (q.answers && q.correctAnswers) {
                    return q;
                }
            }

            // Go up the tree (question usually in parent components)
            if (f.return) {
                const result = searchForQuestion(f.return, depth + 1);
        ...
        return searchForQuestion(fiber);
    }

    // Helper to find Phaser game instance (for Monster Brawl, Tower Defense, etc.)
    // Updated based on tracker findings - game is NOT in window.game
    function getPhaserGame() {
        // Method 1: Check standard window property
        if (window.game && window.game instanceof Phaser.Game) return window.game;

        // Method 2: Check Phaser.GAMES array (Phaser 3.x stores games here)
        if (typeof Phaser !== 'undefined' && Phaser.GAMES && Phaser.GAMES.length > 0) {
            return Phaser.GAMES[0];
        }

        // Method 3: Search from #phaser-game div (where canvas lives per tracker)
        const phaserDiv = document.querySelector('#phaser-game');
        if (phaserDiv) {
            const fiberKey = Object.keys(phaserDiv).find(k => k.startsWith('__reactFiber'));
            if (fiberKey) {
                let fiber = phaserDiv[fiberKey];
                const visited = new WeakSet();
                let foundGame = null;

                function search(f, depth = 0) {
                    if (!f || depth > 30 || visited.has(f) || foundGame) return;
                    visited.add(f);

                    // Check memoizedState for refs containing game
                    if (f.memoizedState) {
                        let state = f.memoizedState;
                        while (state) {
                            if (state.memoizedState?.current instanceof Phaser.Game) {
                                foundGame = state.memoizedState.current;
                                return;
                            }
                            if (state.memoizedState?.game instanceof Phaser.Game) {
                                foundGame = state.memoizedState.game;
                                return;
                            }
                            state = state.next;
                        }
                    ...
                    // Check stateNode for game ref
                    if (f.stateNode && f.stateNode.game instanceof Phaser.Game) {
                        foundGame = f.stateNode.game;
                        return;
                    }

                    if (f.child) search(f.child, depth + 1);
                    if (f.return) search(f.return, depth + 1);
                }
                search(fiber);
    ...
    // NEW: Get Monster Brawl game state from React Fiber props
    // Based on tracker findings: state is at depth 2-3 from _levelText element
    function getBrawlState() {
        const el = document.querySelector('[class*="_levelText"]') ||
            document.querySelector('[class*="_totalXp"]') ||
            document.querySelector('[class*="_gameOverContainer"]');
        if (!el) return null;
        ...
        const result = {
            ...
            levelUpAbility: null,
            levelUpOptions: null,
            fiber: null // Store the fiber for direct manipulation
        };

        // Traverse up to find props (tracker showed depth 2-3)
        let current = fiber;
        let depth = 0;
        while (current && depth < 10) {
            if (current.memoizedProps) {
                const props = current.memoizedProps;
                if (props.xp !== undefined) result.xp = props.xp;
                if (props.level !== undefined) result.level = props.level;
                if (props.curXp !== undefined) result.curXp = props.curXp;
                if (props.totalXp !== undefined) result.totalXp = props.totalXp;
                if (props.gameState !== undefined) result.gameState = props.gameState;
                if (props.player !== undefined) result.player = props.player;
                if (props.gameData !== undefined) result.gameData = props.gameData;
                if (props.levelUpAbility !== undefined) result.levelUpAbility = props.levelUpAbility;
                if (props.levelUpOptions !== undefined) result.levelUpOptions = props.levelUpOptions;

                // If we found the key props, store this fiber
                if (props.levelUpAbility || props.player) {
                    result.fiber = current;
                }
            }
            current = current.return;
            depth++;
    ...
    // Helper to find LiveGameController (Network/State manager) specifically
    function getLiveGameController() {
        const selectors = ['div[class*="_levelText"]', 'div[class*="_totalXp"]', '#app', 'body'];
        for (const sel of selectors) {
            ...
            // Traverse up/down specifically looking for liveGameController
            let controller = null;
            const visited = new WeakSet();
            function search(curr, depth = 0) {
                if (!curr || depth > 50 || visited.has(curr) || controller) return;
                visited.add(curr);

                if (curr.memoizedProps?.liveGameController) {
                    controller = curr.memoizedProps.liveGameController;
                    return;
                }
                if (curr.stateNode?.props?.liveGameController) {
                    controller = curr.stateNode.props.liveGameController;
                    return;
                }

                if (curr.return) search(curr.return, depth + 1);
                if (curr.child) search(curr.child, depth + 1);
                if (curr.sibling) search(curr.sibling, depth + 1);
            }
            search(f);
    ...
    // Helper to find Factory mode functions (upgrade, choose, etc.)
    function getFactoryFunctions() {
        const wrapper = document.querySelector('.wrapper') || document.querySelector('#app');
        const fiberKey = Object.keys(wrapper).find(k => k.startsWith('__reactFiber'));
        let fiber = fiberKey ? wrapper[fiberKey] : Object.values(wrapper)[0];
        ...
        function search(f, depth = 0) {
            ...
            if (f.memoizedProps) {
                ...
                if (p.upgradeFactory || p.onUpgrade) result.upgrade = p.upgradeFactory || p.onUpgrade;
                if (p.deleteFactory || p.onDelete) result.delete = p.deleteFactory || p.onDelete;
                if (p.onChoose) result.choose = p.onChoose;
                if (p.factories) result.factories = p.factories;
                if (p.glitches) result.glitches = p.glitches; // array
                if (p.addCash) result.addCash = p.addCash;
                if (p.sendGlitch) result.sendGlitch = p.sendGlitch;
                if (p.buyGlitch) result.sendGlitch = p.buyGlitch;
                if (p.purchaseGlitch) result.sendGlitch = p.purchaseGlitch;
                if (p.choices) result.choices = p.choices;
            }

            if (f.child) search(f.child, depth + 1);
            if (f.sibling) search(f.sibling, depth);
        }
    ...
    // Robust helper function to find React component state
    // Supports both Colyseus (new) and Firebase (legacy) games
    function getReactOwner(returnStateNode = true) {
        // Try Colyseus first (new games like Factory)
        const room = getColyseusRoom();
        if (room && room.state) {
            // Get question from fiber tree
            const question = getQuestion();
            const player = getCurrentPlayer();
            const phaserGame = getPhaserGame();

            // Return a wrapper that looks like the old stateNode
            // with compatible properties for cheats
            return {
                room: room,
                state: {
                    ...(player || {}), // Include player local state (score, custom props)
                    ...room.state,
                    question: question,
                    stage: room.state.phase,
                    choices: player && player.choices ? player.choices : []
                },
                players: room.state.players,
                currentPlayer: player,
                game: { current: phaserGame },
                send: (type, data) => room.send(type, data), // This is likely empty for Phaser games but good to have
                ...
                props: {
                    ...
                    liveGameController: { send: (type, data) => room.send(type, data) }
                }
            };
        }

        // For React Hooks, state is stored in memoizedState as a linked list
        function extractHooksState(fiber) {
            ...
            while (hookNode && states.length < 20) {
                if (hookNode.memoizedState !== undefined && hookNode.memoizedState !== null) {
                    if (hookNode.memoizedState && hookNode.memoizedState.current !== undefined) {
                        states.push(hookNode.memoizedState.current);
                    } else {
        ...
        // Check if object contains game-related properties
        function hasGameProperties(obj) {
            if (!obj || typeof obj !== 'object') return false;
            return !!(
                obj.liveGameController ||
                obj.client ||
                obj.game ||
                obj.state?.question ||
                obj.state?.stage ||
                obj.props?.liveGameController ||
                obj.current?.scene ||
                obj.unlocks ||
                obj.blookData
            );
        ...
        // Search fibers for game state
        const visited = new WeakSet();
        function findGameStateFromFiber(fiber, depth = 0) {
            if (!fiber || depth > 30 || visited.has(fiber)) return null;
            visited.add(fiber);

            if (fiber.memoizedProps && hasGameProperties(fiber.memoizedProps)) {
                return fiber.memoizedProps;
            }

            if (fiber.stateNode && typeof fiber.stateNode === 'object' &&
                fiber.stateNode !== window && fiber.stateNode.nodeType !== 1) {
                if (hasGameProperties(fiber.stateNode)) {
                    return fiber.stateNode;
                }
            }

            const hooksState = extractHooksState(fiber);
            if (hooksState) {
                for (const state of hooksState) {
                    if (hasGameProperties(state)) {
                        return state;
                    }
                }
            }
            ...
            if (fiber.return && !visited.has(fiber.return)) {
                const result = findGameStateFromFiber(fiber.return, depth + 1);
                if (result) return result;
            }
        ...
        // Search game elements
        const gameSelectors = [
            '#app', '#root', '[class*="_wrapper"]', '[class*="_body"]',
            '[class*="_container"]', '[class*="_game"]', 'main', '.wrapper'
        ];
        ...
        for (const selector of gameSelectors) {
            try {
                ...
                if (fiberKey) {
                    visited.clear();
                    const gameState = findGameStateFromFiber(el[fiberKey]);
                    if (gameState) return gameState;
                }

                for (let i = 0; i < Math.min(el.children.length, 5); i++) {
                    const childFiberKey = getReactFiberKey(el.children[i]);
                    if (childFiberKey) {
                        visited.clear();
                        const gameState = findGameStateFromFiber(el.children[i][childFiberKey]);
                        if (gameState) return gameState;
                    }
                }
            } catch (e) { }
        ...
        // Legacy: recursive stateNode search
        function recursiveSearch(element = document.querySelector("body>div"), depth = 0) {
            if (!element || depth > 10) return null;
            try {
                const reactData = Object.values(element)[1];
                if (reactData?.children?.[0]?._owner?.stateNode) {
                    return element;
                }
            } catch (e) { }
            return recursiveSearch(element.querySelector(":scope>div"), depth + 1);
        }

        const legacySelectors = [
            "#app>div>div", "#root>div>div", "#app>div",
            "#root>div", "body>div>div>div", "body>div>div"
        ];

        let foundElement = recursiveSearch();
        if (!foundElement) {
            for (const selector of legacySelectors) {
                try {
                    const el = document.querySelector(selector);
                    if (el) {
                        const reactData = Object.values(el)[1];
                        if (reactData?.children?.[0]?._owner) {
                            foundElement = el;
                            break;
                        }
                    }
        ...
        if (foundElement) {
            try {
                const owner = Object.values(foundElement)[1].children[0]._owner;
                return returnStateNode ? owner.stateNode : owner;
            } catch (e) { }
        }

        console.error("[Blooket Cheats] Could not find React component. Game mode may not be supported.");
        return null;
    }

    // Alias for backward compatibility
    function getStateNode() {
        return getReactOwner(true);
    }
    ...
    let C = {
        global: [{
            name: "Load Answers (Bypass)",
            description: "Load answers for server-validated modes (Factory)",
            run: async function () {
                const room = getColyseusRoom();
                const instanceId = room?.state?.gameId || room?.state?.config?.gameId;

                if (!instanceId) { alert("No Game ID found"); return; }

                // Helper to find potential IDs
                function scanForIds(rootString) {
                    const found = new Set();
                    ...
                    function walk(node, d = 0) {
                        ...
                        if (node.memoizedProps) {
                            Object.entries(node.memoizedProps).forEach(([k, v]) => {
                                if (typeof v === 'string' && mongoId.test(v) && v !== rootString) found.add(v);
                                if (k === 'setId' || k === 'questionSetId' || k === 'hostId') {
                                    if (v && mongoId.test(v)) found.add(v);
                                }
                            });
                        }
                        if (node.child) walk(node.child, d + 1);
                        if (node.sibling) walk(node.sibling, d + 1);
                    }
                    walk(fiber);
                    return Array.from(found);
                }

                const candidates = scanForIds(instanceId);

                let promptMsg = `Step 1: Find Set ID.\nThe Game ID (${instanceId}) failed API lookup (404).\n\nI found these POTENTIAL Set IDs in memory.\nTry them one by one:\n\n`;

                if (candidates.length === 0) {
                    promptMsg += "(No other IDs found. You might be playing a random set?)\n\n";
                } else {
                ...
                promptMsg += `\nEnter the ID you want to try (or paste a Set ID you know):`;

                // Allow user to pick from list or paste
                let chosenId = prompt(promptMsg, candidates[0] || "");
                if (!chosenId) return;

                // Step 2: Get Answers
                const url2 = `https://api.blooket.com/api/games?gameId=${chosenId}`;
                let json2 = prompt(`Step 2: Get Answers for ID ${chosenId}.\n\n1. Copy URL below\n2. Open in new tab\n3. Copy the JSON text\n4. Paste it here:`, url2);

                if (!json2) return;
                if (json2.trim().startsWith("http")) {
                    json2 = prompt("Please paste the actual JSON content:");
                    if (!json2) return;
                }
                ...
                try {
                    ...
                    if (data && data.questions) {
                        window.blooketAnswers = {};
                        data.questions.forEach(q => {
                            window.blooketAnswers[q.question] = q.correctAnswers;
                        });
                        alert(`✅ Success! Loaded ${data.questions.length} questions for Set ${chosenId}.`);
                    } else {
                        alert("❌ Invalid JSON. Maybe that ID was wrong? Try another candidate.");
                    }
                } catch (e) {
            ...
            run: function () {
                this.enabled ? (this.enabled = !1, clearInterval(this.data), this.data = null) : (this.enabled = !0, this.data = setInterval(() => {
                    ...
                    if (!r) return;
                    try {
                        const correctAnswers = r.correctAnswers || (window.blooketAnswers && window.blooketAnswers[r.question]);
                        if (correctAnswers) {
                            "typing" != r.qType ? ("feedback" === t || a ? document.querySelector('[class*="feedback"]')?.firstChild : [...document.querySelectorAll('[class*="answerContainer"]')][r.answers.map((e, t) => correctAnswers.includes(e) ? t : null).filter(e => null != e)[0]])?.click?.() : Object.values(document.querySelector("[class*='typingAnswerWrapper']"))[1].children._owner.stateNode.sendAnswer(correctAnswers[0])
                        }
                    } catch { }
                }, 50))
            }
            ...
            run: function () {
                this.enabled ? (this.enabled = !1, clearInterval(this.data), this.data = null) : (this.enabled = !0, this.data = setInterval(() => {
                    const stateNode = getStateNode();
                    if (!stateNode) return;
                    const { state: e, props: t } = stateNode;
                    const q = e.question || t.client.question;
                    const correctAnswers = q && (q.correctAnswers || (window.blooketAnswers && window.blooketAnswers[q.question]));
                    if (correctAnswers) {
                        [...document.querySelectorAll('[class*="answerContainer"]')].forEach((a, o) => {
                            correctAnswers.includes(q.answers[o]) ? a.style.backgroundColor = "rgb(0, 207, 119)" : a.style.backgroundColor = "rgb(189, 15, 38)"
                        })
                    }
                }, 50))
            }
            ...
            run: function () {
                this.enabled ? (this.enabled = !1, clearInterval(this.data), this.data = null) : (this.enabled = !0, this.data = setInterval(() => {
                    ...
                    if (correctAnswers) {
                        [...document.querySelectorAll('[class*="answerContainer"]')].forEach((a, o) => {
                            correctAnswers.includes(q.answers[o]) && (a.style.boxShadow = "unset")
                        })
                    }
                }, 50))
            }
            ...
            run: function (e) {
                ...
                document.head.appendChild(t),
                    ...
                    t.remove();
                const owner = getReactOwner(false);
                if (!owner) return;
                owner.stateNode.props.liveGameController.setVal({
                    path: "c/" + owner.stateNode.props.client.name + "/bg",
                    val: e
                })
            }
        }, {
            ...
            run: async function (e, t, a) {
                ...
                document.body.append(o),
                    ...
                    o.remove();
                let r = getStateNode();
                if (!r) return alert("Could not find game state!");
            ...
            run: async (packName) => {
                ...
                const stateNode = getStateNode();
                if (!stateNode) {
                    alert("Could not find game state!");
                    return;
                }
            ...
            run: function () {
                ...
                const originalSetState = stateNode.updater.enqueueSetState.bind(stateNode.updater);
                stateNode.setState = function (a, b) {
                    if (a?.takenBlooks) {
                        return;
                    }
                    originalSetState(stateNode, a, b, "setState");
                }
            }
        }, {
        ...
        voyage: [{
            ...
            run: function () {
                ...
                function setv(args) {
                    const owner = reactHandler();
                    if (!owner) return;
                    owner.stateNode.props.liveGameController.setVal({
                        path: "c/" + owner.stateNode.props.client.name + "/" + args[0],
            ...
            run: function (userInput) {
                ...
                function setValForPlayer() {
                    ...
                    if (!owner) return;
                    owner.stateNode.props.liveGameController.getDatabaseVal("c/").then(data => {
                        if (data != null) {
        ...
        brawl: [{
            ...
            description: "Check what game data is accessible",
            run: function () {
                const brawl = getBrawlState();
                const game = getPhaserGame();

                let msg = "=== BRAWL DIAGNOSTIC ===\n";
                msg += "Phaser Game: " + (game ? "FOUND" : "NOT FOUND") + "\n";
                msg += "Brawl State: " + (brawl ? "FOUND" : "NOT FOUND") + "\n";

                if (brawl) {
                    msg += "\n--- State Data ---\n";
                    msg += "XP: " + brawl.xp + "\n";
                    msg += "Level: " + brawl.level + "\n";
                    msg += "CurXP: " + brawl.curXp + "\n";
                    msg += "TotalXP: " + brawl.totalXp + "\n";
                    msg += "GameState: " + brawl.gameState + "\n";
                    msg += "Player: " + (brawl.player ? "FOUND" : "NOT FOUND") + "\n";
                    msg += "LevelUpAbility: " + (brawl.levelUpAbility ? "FOUND" : "NOT FOUND") + "\n";
                }

                alert(msg);
                console.log("Brawl State:", brawl);
                console.log("Phaser Game:", game);
            }
        }, {
            name: "Force Level Up",
            description: "Triggers level up with random ability",
            run: function () {
                const brawl = getBrawlState();
                if (!brawl || !brawl.levelUpAbility) {
                    C.alerts[0].addLog("Error: levelUpAbility not found");
                    return;
                }

                // Call the levelUpAbility function with a random ability ID
                // Common abilities: speed, damage, health, range, etc.
                const abilities = ["speed", "damage", "health", "range", "luck", "multishot"];
                const randomAbility = abilities[Math.floor(Math.random() * abilities.length)];

                try {
                    brawl.levelUpAbility(randomAbility);
                    C.alerts[0].addLog("Leveled Up: " + randomAbility);
                } catch (e) {
                    console.error(e);
                    C.alerts[0].addLog("Error: " + e.message);
                }
            }
        }, {
            name: "Get Player Object",
            description: "Logs the player object to console for inspection",
            run: function () {
                const brawl = getBrawlState();
                if (!brawl || !brawl.player) {
                    C.alerts[0].addLog("Error: Player not found");
                    return;
                }

                console.log("=== PLAYER OBJECT ===");
                console.log(brawl.player);
                console.log("Player Keys:", Object.keys(brawl.player));

                // Try to list all properties
                let props = [];
                for (let key in brawl.player) {
                    props.push(key + ": " + typeof brawl.player[key]);
                }
                console.log("Properties:", props.join(", "));

                C.alerts[0].addLog("Player logged to console (F12)");
                alert("Player object logged to console.\nOpen DevTools (F12) to see details.\n\nKeys: " + Object.keys(brawl.player).slice(0, 10).join(", ") + "...");
            }
        }, {
            name: "Show Game State",
            description: "Shows current game state",
            run: function () {
                const brawl = getBrawlState();
                if (!brawl) {
                    C.alerts[0].addLog("Error: State not found");
                    return;
                }
                ...
                alert(
                    ...
                    "XP: " + brawl.xp + "\n" +
                    "Level: " + brawl.level + "\n" +
                    "Total XP: " + brawl.totalXp + "\n" +
                    "Game State: " + brawl.gameState
                );
            }
        }, {
            ...
            run: function () {
                const game = getPhaserGame();
                if (!game) {
                    C.alerts[0].addLog("Error: Phaser Game Not Found");
                    return;
                }
                const scene = game.scene?.scenes?.[0];
                if (!scene) {
                    C.alerts[0].addLog("Error: Scene Not Found");
                    return;
                }

                try {
                    const colliders = scene.physics.world.colliders._active;
                    const enemyOverlap = colliders.find(e => e.callbackContext?.toString().includes("invulnerableTime"));

                    if (enemyOverlap) {
                        const enemyGroup = enemyOverlap.object2;
                        const originalStart = enemyGroup.classType.prototype.start;

                        enemyGroup.classType.prototype.start = function () {
                            if (originalStart) originalStart.apply(this, arguments);
                            this.val *= 2;
                        };

                        // Update existing enemies
                        if (enemyGroup.children && enemyGroup.children.entries) {
                            enemyGroup.children.entries.forEach(e => e.val *= 2);
                        }
                        C.alerts[0].addLog("Double XP Enabled!");
                    } else {
                        C.alerts[0].addLog("Error: Enemy Collider Not Found");
                        // Fallback: Try to find any group with 'enemy' in name or logical inference
                        // This is a common failure point if obfuscation changed
                    }
                } catch (err) {
                    console.error(err);
                    C.alerts[0].addLog("Error: " + err.message);
                }
            }
        }, {
            ...
            run: function () {
                const game = getPhaserGame();
                if (!game || !game.scene || !game.scene.scenes || !game.scene.scenes[0]) return;

                const scene = game.scene.scenes[0];
                const colliders = scene.physics.world.colliders._active;

                const relevantColliders = colliders.filter(e => e.callbackContext?.toString().includes("invulnerableTime"));

                relevantColliders.forEach(c => {
                    const enemyGroup = c.object2;
                    if (enemyGroup && enemyGroup.classType) {
                        const originalStart = enemyGroup.classType.prototype.start;
                        enemyGroup.classType.prototype.start = function () {
                            if (originalStart) originalStart.apply(this, arguments);
                            this.speed *= 0.5;
                        ...
                        if (enemyGroup.children && enemyGroup.children.entries) {
                            enemyGroup.children.entries.forEach(e => e.speed *= 0.5);
                        }
                    }
                });
                C.alerts[0].addLog("Half Enemy Speed Enabled");
            }
        }, {
            name: "Instant Kill",
            ...
            run: function () {
                ...
                relevantColliders.forEach(c => {
                    ...
                    if (enemyGroup && enemyGroup.classType) {
                        ...
                        if (enemyGroup.children && enemyGroup.children.entries) {
                            enemyGroup.children.entries.forEach(e => e.hp = 1);
                        }
                    }
                });
                C.alerts[0].addLog("Instant Kill Enabled");
            }
        }, {
            name: "Invincibility",
            ...
            run: function () {
                ...
                let found = false;
                colliders.forEach(e => {
                    if (e.callbackContext && e.callbackContext.toString().includes("invulnerableTime")) {
                        e.collideCallback = () => { }; // Disable collision callback
                        found = true;
                    }
                });

                if (found) C.alerts[0].addLog("Invincibility Enabled");
                else C.alerts[0].addLog("Could not find player collider");
            }
        }, {
            name: "Magnet",
            ...
            run: function () {
                ...
                // Find magnet interaction
                const collider = scene.physics.world.colliders._active.find(e => e.collideCallback?.toString().includes("magnetTime"));

                if (collider && collider.collideCallback) {
                    // Trigger magnet effect manually or force it
                    // This function seems to stimulate the callback with a fake object 'active: true'
                    collider.collideCallback({ active: true }, {
                        active: true,
                        setActive: () => { },
                        setVisible: () => { }
                    });
                    C.alerts[0].addLog("Magnet Activated");
                }
            }
        }, {
            ...
            run: function () {
                const game = getPhaserGame();
                const stateNode = getStateNode() || getReactOwner()?.stateNode;

                if (!game || !stateNode) return;
                // Need game scene for events
                const scene = game.scene.scenes[0];
                if (!scene) return;

                if (stateNode.state && stateNode.state.abilities) {
                    for (var [t, a] of Object.entries(stateNode.state.abilities)) {
                        for (let o = 0; o < 10 - a; o++) {
                            // Emit level up event
                            scene.game.events.emit("level up", t, stateNode.state.abilities[t]++);
                        }
                    }

                    // Force level update logic
                    // This logic seems complex, preserving original intent
                    const levels = [1, 3, 5, 10, 15, 25, 35];
                    const nextLevel = levels.sort((a, b) => Math.abs(a - stateNode.state.level) - Math.abs(b - stateNode.state.level))[0] - 1;

                    stateNode.setState({ level: nextLevel });
                    scene.level = nextLevel;

                    C.alerts[0].addLog("Maxed Abilities");
                }
            }
        }, {
            name: "Next Level",
            ...
            run: function () {
                ...
                const xpCollider = scene.physics.world.colliders._active.find(e => e.collideCallback?.toString().includes('emit("xp"'));

                if (xpCollider) {
                    const object1 = xpCollider.object1; // Usually player
                    const object2 = xpCollider.object2; // Usually XP group

                    if (object2 && stateNode && stateNode.state) {
                        const level = stateNode.state.level;
                        const xpNeeded = (level === 1 ? 1 : level < 5 ? 5 : level < 10 ? 10 : level < 20 ? 20 : level < 30 ? 30 : level < 40 ? 40 : level < 50 ? 50 : 100);
                        const currentXp = stateNode.xp || 0;

                        // Spawn XP enough to level up
                        object2.get().spawn(object1.x, object1.y, xpNeeded - currentXp);
                        C.alerts[0].addLog("Spawned Level Up XP");
                    }
                }
            }
        }, {
            ...
            run: function () {
                const game = getPhaserGame();
                if (!game || !game.scene || !game.scene.scenes || !game.scene.scenes[0]) return;
                const world = game.scene.scenes[0].physics.world;

                let removed = 0;
                world.bodies.entries.forEach(e => {
                    if (e.gameObject && e.gameObject.frame && e.gameObject.frame.texture.key.includes("obstacle")) {
                        e.gameObject.destroy();
                        removed++;
                    }
                });
                C.alerts[0].addLog("Removed " + removed + " Obstacles");
            }
        }, {
            name: "Kill Enemies",
            ...
            run: function () {
                ...
                let killed = 0;
                world.bodies.entries.forEach(e => {
                    if (e && e.gameObject && e.gameObject.getData('type') === 'enemy') {
                        e.gameObject.receiveDamage(9999);
                        killed++;
                    } else if (e && e.gameObject && e.gameObject.receiveDamage) {
                        // Fallback for objects that look like enemies
                        e.gameObject.receiveDamage(9999);
                        killed++;
                    }
                });
                C.alerts[0].addLog("Killed " + killed + " Enemies");
            }
        }, {
            name: "Reset Health",
            description: "Resets health",
            run: function () {
                const game = getPhaserGame();
                if (!game || !game.scene || !game.scene.scenes || !game.scene.scenes[0]) return;
                try {
                    game.scene.scenes[0].events.emit("respawn");
                    C.alerts[0].addLog("Health Reset");
                } catch (e) {
                    console.error("Reset Health failed", e);
                }
            }
        }, {
            name: "Set XP",
            ...
            run: function (e) {
                // Try Phaser first for Monster Brawl
                const game = getPhaserGame();
                if (game && game.scene && game.scene.scenes && game.scene.scenes[0]) {
                    const scene = game.scene.scenes[0];
                    // Update Phaser level/xp if possible
                    // Monster Brawl usually tracks this in stateNode, but let's try to find where it reads XP
                    // Often it's purely reactive to stateNode.
                }

                var t = getStateNode();
                if (!t) {
                    const owner = getReactOwner();
                    t = owner ? owner.stateNode : null;
                }
                ...
                if (t) {
                    ...
                    if (t.setState) {
                        ...
                        C.alerts[0].addLog("XP set locally (Client-Side Only)");
                    } else {
                        t.xp = e;
                        t.totalXp = e;
                    }
                } else {
                    C.alerts[0].addLog("Could not find Game State");
                }
            }
        }, {
            name: "Set Level",
            description: "Sets your level (Client Side)",
            inputs: [{
                name: "Level",
                type: "number"
            }],
            run: function (e) {
                var t = getStateNode();
                if (!t) t = getReactOwner()?.stateNode;

                if (t && t.setState) {
                    t.setState({ level: e });
                    C.alerts[0].addLog("Level set locally");
                }
            }
        }
        ],
        ...
        factory: [{
            ...
            run: function (e) {
                ...
                try {
                    ...
                    // Ensure all required properties exist to prevent toString crash
                    const newBlook = {
                        ...parsed,
                        level: 4, // Default to max level
                        bonus: 1,
                        active: false,
                        time: parsed.time || [1, 1, 1, 1, 1], // fallback
                        cash: parsed.cash || [1, 1, 1, 1, 1],
                        price: parsed.price || [0, 0, 0, 0],
                        timer: 0
                    };

                    const funcs = getFactoryFunctions();

                    if (funcs.factories) {
                        // Direct state manipulation
                        if (funcs.factories.length < 10) {
                            funcs.factories.push(newBlook);
                            C.alerts[0].addLog("Blook Added");
                        } else {
                            // If full, replace slot 10
                            funcs.factories[9] = newBlook;
                            C.alerts[0].addLog("Blook Replaced (Slot 10)");
                        }
                    } else {
                        // Fallback to legacy
                        const a = getStateNode();
                        if (a && a.chooseBlook) {
                            a.chooseBlook(newBlook);
                        } else if (a && a.state && a.state.blooks) {
                            // Legacy state manipulation
                            const blooks = [...a.state.blooks];
                            if (blooks.length < 10) blooks.push(newBlook);
                            else blooks[9] = newBlook;
                            a.setState({ blooks });
                        }
                    }
                } catch (err) {
                    console.error("Choose blook error:", err);
                }
            }
        }, {
            name: "Free Upgrades",
            ...
            run: function () {
                const funcs = getFactoryFunctions();
                if (funcs.factories) {
                    funcs.factories.forEach(f => {
                        // Set price to 0 for all levels
                        // We set it to an array of 0s corresponding to upgrade levels
                        if (Array.isArray(f.price)) {
                            f.price = f.price.map(() => 0);
                        } else {
                            f.price = [0, 0, 0, 0];
                        }
                    });
                    C.alerts[0].addLog("Free Upgrades Enabled (Prices Set to 0)");
                } else {
                    const stateNode = getStateNode();
                    if (stateNode && stateNode.setState) {
                        stateNode.setState(e => ({
                            ...e,
            ...
            run: function () {
                ...
                if (funcs.factories) {
                    funcs.factories.forEach((f) => {
                        // Directly set level to max (usually 4)
                        f.level = 4;
                    });
                    C.alerts[0].addLog("Blooks Maxed Out");
                } else {
                    const stateNode = getStateNode();
                    if (stateNode && stateNode.state && stateNode.state.blooks) {
                        stateNode.state.blooks.forEach(e => e.level = 4);
                        if (stateNode.forceUpdate) stateNode.forceUpdate();
                    }
                }
            }
        }, {
            ...
            run: function () {
                const funcs = getFactoryFunctions();
                let fixed = false;
                if (funcs.glitches && Array.isArray(funcs.glitches)) {
                    funcs.glitches.length = 0; // Clear array in place
                    fixed = true;
                }

                // Always try fallback as well since state might be split
                const e = getStateNode();
                if (e && e.setState) {
                    e.setState({
                        bits: 0,
                        ads: [],
                ...
                if (fixed) C.alerts[0].addLog("Removed Glitches");
            }
        }, {
            name: "Set Next Choice",
            description: "Forces a specific blook to appear in your next choice selection",
            inputs: [{
                name: "Blook Name",
                type: "text",
                value: "Mega Bot" // Default
            }],
            run: function (e) {
                const funcs = getFactoryFunctions();
                const targetName = e || "Mega Bot";
                const targetBlook = {
                    name: targetName, color: "#d71f27", class: "\uD83E\uDD16", rarity: "Legendary",
                    cash: [1e9, 1e9, 1e9, 1e9, 1e9], time: [1, 1, 1, 1, 1], price: [0, 0, 0, 0],
                    active: false, level: 4, bonus: 5.5, timer: 0
                };

                if (funcs.choices && Array.isArray(funcs.choices)) {
                    // Overwrite the first choice
                    funcs.choices[0] = targetBlook;
                    // Force update if possible, usually requires waiting for React to re-render or just click
                    C.alerts[0].addLog(`Next choice set to ${targetName}. Open choice menu!`);
                } else {
                    // Fallback to stateNode
                    const a = getStateNode();
                    if (a && a.state && a.state.choices) {
                        const newChoices = [...a.state.choices];
                        newChoices[0] = targetBlook;
                        a.setState({ choices: newChoices });
                        C.alerts[0].addLog(`Next choice set to ${targetName} (Legacy)`);
                    } else {
                        C.alerts[0].addLog("Could not find 'choices' prop. Make sure you are in game.");
                    }
                }
            }
        }, {
            name: "Give Glitch (Inventory)",
            description: "Adds a glitch to your inventory so you can send it manually",
            inputs: [{
                name: "Glitch",
                type: "options",
                options: Object.entries({
            ...
            run: function (e) {
                const funcs = getFactoryFunctions();
                const glitchMap = {
                    lb: { name: "Lunch Break", type: "glitch", class: "fa-mug-hot" },
                    as: { name: "Ad Spam", type: "glitch", class: "fa-ad" },
                    e37: { name: "Error 37", type: "glitch", class: "fa-exclamation-triangle" },
                    nt: { name: "Night Time", type: "glitch", class: "fa-moon" },
                    lo: { name: "#LOL", type: "glitch", class: "fa-laugh-squint" },
                    j: { name: "Jokester", type: "glitch", class: "fa-theater-masks" },
                    sm: { name: "Slow Mo", type: "glitch", class: "fa-hourglass-half" },
                    dp: { name: "Dance Party", type: "glitch", class: "fa-music" },
                    v: { name: "Vortex", type: "glitch", class: "fa-fan" },
                    r: { name: "Reverse", type: "glitch", class: "fa-history" },
                    f: { name: "Flip", type: "glitch", class: "fa-random" },
                    m: { name: "Micro", type: "glitch", class: "fa-compress-arrows-alt" }
                };

                const glitchObj = glitchMap[e];
                if (!glitchObj) return;

                if (funcs.glitches && Array.isArray(funcs.glitches)) {
                    funcs.glitches.push(glitchObj);
                    C.alerts[0].addLog(`Added ${glitchObj.name} to inventory!`);
                } else {
                    C.alerts[0].addLog("Could not find glitch inventory.");
                    // Legacy fallback
                    const a = getStateNode();
                    if (a && a.state && a.state.glitches) {
                        a.setState({ glitches: [...a.state.glitches, glitchObj] });
                    }
                }
            }
        }, {
            // Keep old Send Glitch for debugging if needed, but renamed/hidden
            name: "Send Glitch (Unsafe)",
            description: "Directly sends a glitch (Can cause disconnects)",
            inputs: [{
                name: "Glitch",
                type: "options",
                options: Object.entries({
            ...
            run: function (e) {
                ...
                if (funcs.sendGlitch) {
                    funcs.sendGlitch(e);
                    C.alerts[0].addLog(`Sent Glitch: ${e}`);
                } else {
                    C.alerts[0].addLog("Glitch function not found. Check Console.");
                    console.log("Factory Functions Found:", Object.keys(funcs));
                    console.log("Could not find sendGlitch/buyGlitch prop. Network send disabled to prevent ban.");
                }
            }
        }, {
            name: "Add Mega Bots",
            description: "Adds a specific number of fully upgraded Mega Bots",
            inputs: [{
                name: "Amount",
                type: "number"
            }],
            run: function (e) {
                ...
                const funcs = getFactoryFunctions();
                const megaBotTemplate = {
                    name: "Mega Bot", color: "#d71f27", class: "\uD83E\uDD16", rarity: "Legendary",
                    cash: [8e4, 43e4, 42e5, 62e6, 1e9], time: [5, 5, 3, 3, 3], price: [0, 0, 0, 0], // Free price
                    active: false, level: 4, bonus: 5.5, timer: 0
                };
                ...
                if (funcs.factories) {
                    ...
                    for (let i = 0; i < amount; i++) {
                        if (funcs.factories.length < 10) {
                            ...
                            funcs.factories.push(JSON.parse(JSON.stringify(megaBotTemplate)));
                            added++;
                        } else {
                            // Optional: Break if full, or replace? User asked to "Add".
                            // We will stop if full to be safe/logical.
                            break;
                        }
                    }
                    C.alerts[0].addLog(`Added ${added} Mega Bots`);
                } else {
                    // Legacy Fallback
                    const stateNode = getStateNode();
                    if (stateNode && stateNode.state && stateNode.state.blooks) {
                        const blooks = [...stateNode.state.blooks];
                        let added = 0;
                        for (let i = 0; i < amount; i++) {
                            if (blooks.length < 10) {
                                blooks.push(JSON.parse(JSON.stringify(megaBotTemplate)));
                                added++;
                            } else break;
                        }
                        stateNode.setState({ blooks });
                        C.alerts[0].addLog(`Added ${added} Mega Bots (Legacy)`);
                    } else {
                        C.alerts[0].addLog("Could not find factory state");
                    }
                }
            }
        }, {
            ...
            run: function () {
                ...
                if (funcs.factories) {
                    const megaBot = {
                        name: "Mega Bot", color: "#d71f27", class: "\uD83E\uDD16", rarity: "Legendary",
                        cash: [8e4, 43e4, 42e5, 62e6, 1e9], time: [5, 5, 3, 3, 3], price: [7e6, 12e7, 19e8, 35e9],
                        active: !1, level: 4, bonus: 5.5
                    };
                    for (let i = 0; i < funcs.factories.length; i++) {
                        Object.assign(funcs.factories[i], megaBot); // Update in place
                    }
                } else {
                    const stateNode = getStateNode();
                    if (stateNode && stateNode.setState) {
            ...
            run: function (e) {
                // Find local React state and addCash function
                const wrapper = document.querySelector('.wrapper') || document.querySelector('#app');
                const fiberKey = Object.keys(wrapper).find(k => k.startsWith('__reactFiber'));
                let fiber = fiberKey ? wrapper[fiberKey] : Object.values(wrapper)[0];
                ...
                function searchFiber(f, depth = 0) {
                    ...
                    if (f.memoizedProps) {
                        ...
                        if (typeof props.addCash === 'function') {
                            addCashFn = props.addCash;
                        }
                        if (props.cash !== undefined) {
                            currentCash = props.cash;
                        }
                    }

                    if (!addCashFn && f.child) searchFiber(f.child, depth + 1);
                    if (!addCashFn && f.sibling) searchFiber(f.sibling, depth);
                }

                // Search down from wrapper
                searchFiber(fiber);

                if (addCashFn) {
                    // Calculate difference to "Set" the cash
                    // e is the target value
                    addCashFn(e - currentCash);

                    // Also try to send to Colyseus server directly as backup
                    try {
                        const room = getColyseusRoom();
                        if (room) {
                            room.send("playerSetScore", { score: e });
                        }
                    } catch (err) { }
                } else {
                    // Fallback to legacy
