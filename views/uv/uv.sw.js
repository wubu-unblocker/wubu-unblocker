...
class UVServiceWorker extends Ultraviolet.EventEmitter {
  ...
  async fetch({ request }) {
    ...
    try {
      ...
      // Some BareMux transports/environments don't reliably handle modern content encodings
      // (notably brotli). Force identity for JS/CSS/HTML to avoid garbled bytes causing broken layouts.
      if (
        ['document', 'iframe', 'style', 'script', 'worker', 'sharedworker'].includes(
          request.destination
        )
      ) {
        requestCtx.headers['accept-encoding'] = 'identity';
      }
      // Ensure an Accept header exists for stricter CDNs (can prevent 406 responses).
      if (!requestCtx.headers.accept) {
        switch (request.destination) {
          case 'style':
            requestCtx.headers.accept = 'text/css,*/*;q=0.1';
            break;
          case 'script':
          case 'worker':
          case 'sharedworker':
            requestCtx.headers.accept = '*/*';
            break;
          case 'document':
          case 'iframe':
            requestCtx.headers.accept =
      ...
      let response = await this.bareClient.fetch(fetchedURL, {
      ...
      // Bare client responses may come back with a Content-Encoding applied but not decoded.
      // If we rewrite compressed JS/CSS/HTML as if it were text, the result is broken layout.
      const ce = (response.headers.get('content-encoding') || '').toLowerCase();
      const canDecompress =
        typeof DecompressionStream !== 'undefined' &&
        response.body &&
        ['br', 'gzip', 'deflate'].includes(ce) &&
        ['document', 'iframe', 'style', 'script', 'worker', 'sharedworker'].includes(
          request.destination
        );
      ...
      if (canDecompress) {
        try {
          ...
          // Preserve BareClient-added metadata while updating rawHeaders to match.
          ...
          decompressed.finalURL = response.finalURL;
          decompressed.rawResponse = response.rawResponse;
          decompressed.rawHeaders = rawHeaders;
          response = decompressed;
        } catch (e) {
          // If decompression fails, fall back to original response.
          console.warn('UV: decompression failed, continuing without it:', e);
        }
      }
