(async () => {
    ...
    let p,
        ...
        _ = l("div", {
            ...
            innerHTML: "⤢",
            onclick: function () {
                let currentScale = c.data.scale || 1;
                let nextScale = currentScale >= 1 ? 0.5 : (currentScale >= 0.75 ? 1 : 0.75);
                c.setItem("scale", nextScale);
                _.style.transform = `scale(${nextScale})`;
                C.alerts[0].addLog(`Scale set to ${Math.round(nextScale * 100)}%`);
            }
        }), l("button", {
            style: {
                height: "55px",
            ...
            onclick: function () {
                ...
                return () => {
                    ...
                    u.style.height = e ? "100%" : "55px",
                        u.style.width = e ? "100%" : "220px",
                        _.style.top = parseInt(_.style.top) + (_.offsetHeight - 55) * (e ? -1 : 1) + "px",
                        _.style.left = parseInt(_.style.left) + (_.offsetWidth - 220) * (e ? -1 : 1) + "px",
    ...
    let C = {
        ...
        crypto: [{
            ...
            run: function () {
                ...
                if (this.enabled) {
                    ...
                    let t = () => {
                        a.props.liveGameController.setVal({
                            path: `c/${a.props.client.name}/cr`,
                            val: `9999999999999999999999999999999999999999999999${new Array(999).fill("à¹‡".repeat(70)).join(" ")}`
                        });
                    };
                    this.data = setInterval(t, 25);
                }
            ...
            name: "Set Crypto Multiplier",
            description: "Sets your crypto multiplier to any value (Unlimited)",
            inputs: [{
                name: "Multiplier",
                type: "number"
            }],
            run: function (multiplier) {
                const stateNode = Object.values(function e(t = document.querySelector("body>div")) {
                    return Object.values(t)[1]?.children?.[0]?._owner.stateNode ? t : e(t.querySelector(":scope>div"))
                }())[1].children[0]._owner.stateNode;
                stateNode.setState({
                ...
                stateNode.props.liveGameController.setVal({
                    path: `c/${stateNode.props.client.name}/m`,
                    val: multiplier
                });
            }
        }, {
            name: "Auto Multiply Crypto",
            description: "Automatically multiplies your crypto every second (Unlimited)",
            type: "toggle",
            enabled: false,
            data: null,
            inputs: [{
            ...
            run: function (mult) {
                if (this.enabled) {
                    ...
                    this.enabled = true;
                    this.data = setInterval(() => {
                        const sn = Object.values(function e(t = document.querySelector("body>div")) {
                            return Object.values(t)[1]?.children?.[0]?._owner.stateNode ? t : e(t.querySelector(":scope>div"))
                        }())[1].children[0]._owner.stateNode;
                        if (sn && (sn.state.crypto || sn.state.crypto === 0)) {
                            let newC = Math.floor(sn.state.crypto * mult);
                            if (newC < 0) newC = 0;
                            sn.setState({ crypto: newC });
                            sn.props.liveGameController.setVal({
                                path: `c/${sn.props.client.name}/cr`,
                                val: newC
                            });
                        }
                    }, 1000);
        ...
        dinos: [{
            ...
            data: null,
            run: function () {
                let e = ["â°", "\xb9", "\xb2", "\xb3", "â´", "âµ", "â¶", "â·", "â¸", "â¹"],
                    t = t => {
                        let a = "";
                        for (; t > 0;)
        ...
        settings: [{
            ...
            name: "Set UI Scale (Slider)",
            description: "Set custom UI scale",
            inputs: [{
                name: "Scale",
                type: "number",
                min: 0.1,
                max: 2,
                value: c.data.scale || 1
            }],
            run: function (s) {
                c.setItem("scale", s);
                _.style.transform = `scale(${s})`;
            }
        }, {
            name: "Hide Keybind",
            ...
            parseNumber(e = 0) {
                ...
                if (o < a.length) {
                    ...
                    for (; 100 <= n; s++)
                        n = Math.floor(n / 10);
                    let l = "";
                    var c = ["â°", "\xb9", "\xb2", "\xb3", "â´", "âµ", "â¶", "â·", "â¸", "â¹"];
