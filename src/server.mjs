...
const wuTubeInvidiousSearchInstances = Object.freeze([
...
function wuTubeShortDescription(value, maxLength = 280) {
  const text = wuTubeDescription(value).replace(/\s+/g, ' ').trim();
  if (!text) return '';
...
function wuTubeViews(value) {
  ...
  const text = wuTubeText(value).trim();
  const compact = text.match(/([\d.,]+)\s*([KMB])/i);
  if (compact) {
    const numeric = Number(compact[1].replace(/,/g, ''));
    const factor = { K: 1e3, M: 1e6, B: 1e9 }[compact[2].toUpperCase()] || 1;
    return Number.isFinite(numeric) ? Math.round(numeric * factor) : 0;
  }
...
function wuTubeStripHtml(value) {
  return wuTubeDecodeHtmlEntities(String(value || '').replace(/<[^>]+>/g, ' '))
    .replace(/\s+/g, ' ')
    .trim();
}

function wuTubeExtractVideoId(url) {
  try {
...
function normalizeWuTubeFallbackSearchResult(result) {
  ...
  return {
    ...
    thumbnail: wuTubeThumbnail([], result.videoId),
    views: wuTubeViews(result.viewsText),
    timestamp: result.timestamp || '--:--',
    seconds: 0,
    ago: result.ago || '',
    uploadDate: result.ago || '',
    author: {
      name: result.author || 'Unknown creator',
      url: '',
    },
  };
}

async function wuTubeFetchJinaPage(targetUrl) {
  return await withTimeout(
    fetch(`https://r.jina.ai/http://${targetUrl.replace(/^https?:\/\//, '')}`, {
      headers: {
        'user-agent': 'Mozilla/5.0',
      },
    }).then((response) => {
      if (!response.ok) {
        throw new Error(`Jina mirror failed with HTTP ${response.status}`);
      }
      return response.text();
    }),
    15000,
    `WuTube Jina fetch (${targetUrl})`
  );
}

function wuTubeSearchInstances() {
  const extra = String(process.env.WUTUBE_INVIDIOUS_SEARCH_INSTANCES || '')
    .split(',')
    .map((entry) => entry.trim())
    .filter(Boolean);
  return [...new Set([...extra, ...wuTubeInvidiousSearchInstances])];
}

function wuTubeParseInvidiousSearchResults(html, limit = 12) {
  const results = [];
  const cardBlocks = html
    .split('<div class="pure-u-1 pure-u-md-1-4">')
    .slice(1);

  for (const block of cardBlocks) {
    const videoIdMatch = block.match(/href="\/watch\?v=([^"&]+)(?:[^"]*)"/);
    if (!videoIdMatch) continue;
    const videoId = wuTubeDecodeHtmlEntities(videoIdMatch[1]);
    if (!videoId) continue;

    const titleMatch = block.match(
      /<a href="\/watch\?v=[^"]+">\s*<p[^>]*dir="auto">([\s\S]*?)<\/p>\s*<\/a>/
    );
    ...
    results.push(
      normalizeWuTubeFallbackSearchResult({
        ...
        viewsText,
        ago: agoText,
        timestamp: wuTubeStripHtml(lengthMatch?.[1] || '--:--'),
      })
    );
...
async function wuTubeInvidiousSearch(query, limit = 12) {
  const errors = [];
    ...
    try {
      ...
      const html = await withTimeout(
        fetch(url, {
          ...
          if (!response.ok) {
            throw new Error(`Invidious search failed with HTTP ${response.status}`);
          }
          return response.text();
        }),
        15000,
        `WuTube Invidious search (${url})`
      );

      const videos = wuTubeParseInvidiousSearchResults(html, limit);
      if (videos.length) {
        return { videos, degraded: true, source: baseUrl };
      }
      throw new Error(`No Invidious search results from ${baseUrl}.`);
    } catch (error) {
      errors.push({ baseUrl, error });
    }
  }

  const aggregate = new Error('WuTube Invidious search failed.');
...
async function wuTubeFallbackVideo(videoId) {
  ...
  const data = await withTimeout(
    fetch(noembedUrl, {
      ...
      if (!response.ok) {
        throw new Error(`Noembed failed with HTTP ${response.status}`);
      }
...
async function searchWuTubeVideos(query, limit = 12) {
  try {
    return await wuTubeInvidiousSearch(query, limit);
  } catch (invidiousError) {
    console.error('[WuTube] Invidious search failed, trying mirrored fallback:', invidiousError);
    const html = await wuTubeFetchJinaPage(
    ...
    const videos = titleMatches
      .map((match) => {
        const videoId = wuTubeExtractVideoId(match[2]);
        if (!videoId) return null;
        return normalizeWuTubeFallbackSearchResult({
          videoId,
          url: `https://youtube.com/watch?v=${videoId}`,
          title: wuTubeStripHtml(match[1]),
          description: '',
          author: '',
        });
      })
    ...
    if (!videos.length) {
      throw new Error('WuTube search produced no results.');
    }

    return { videos, degraded: true, source: 'jina-mirror' };
  }
}

async function loadWuTubeVideo(videoId) {
  return {
    video: await wuTubeFallbackVideo(videoId),
    degraded: true,
    source: 'noembed',
  };
}
...
app.get(serverUrl.pathname + 'youtube/api/home', async (req, reply) => {
  const settled = await Promise.allSettled(
    wuTubeHomeQueries.map(async (section) => ({
      ...section,
      ...(await searchWuTubeVideos(section.query, 8)),
    }))
  );
  ...
  const sections = settled
    ...
    .map((section) => ({
      ...
      degraded: Boolean(section.degraded),
  ...
  return reply.send({
    sections,
    degraded:
      sections.length !== wuTubeHomeQueries.length ||
      sections.some((section) => section.degraded),
  });
});
...
app.get(serverUrl.pathname + 'youtube/api/search', async (req, reply) => {
  ...
  try {
    const result = await searchWuTubeVideos(query, 18);
    return reply.send({ query, videos: result.videos, degraded: Boolean(result.degraded) });
  } catch (e) {
...
app.get(serverUrl.pathname + 'youtube/api/video/:id', async (req, reply) => {
  try {
    const result = await loadWuTubeVideo(req.params.id);
    const video = result.video;
    const relatedQuery = `${video.author.name} ${video.title}`.trim();
    let related = [];
    let degraded = Boolean(result.degraded);

    try {
      const relatedResult = await searchWuTubeVideos(relatedQuery, 10);
      related = relatedResult.videos.filter(
        (candidate) => candidate.videoId !== video.videoId
      );
      degraded = degraded || Boolean(relatedResult.degraded);
    } catch (error) {
      degraded = true;
