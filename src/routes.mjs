...
const ecosystemConfig = Object.freeze(
  ecosystem.apps.find((app) => app.name === 'WubuUnblocker') || ecosystem.apps[0]
);
...
let pages = {
  ...
  default: config.disguiseFiles ? 'login' : 'stuff',
  index: 'index.html',
  stuff: '../stuff.html', // Wubu main hub
...
/* Helper to safely read file or return empty string if missing (during build) */
...
const insert = JSON.parse(
  ...
  text404 = safeRead('../views/' + pages['test-404']),
  uvError = safeRead('../views/' + pages['uverror']),
  sjError = safeRead('../views/' + pages['sjerror']);
